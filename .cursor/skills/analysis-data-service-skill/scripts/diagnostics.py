    ...
    for module_name, label in deps:
        ...
        results.append(
            CheckResult(
                ...
                action="" if exists else f"执行 pip install {module_name} 安装缺失依赖。",
            )
        )
    return results
...
def run_runtime_checks(
    org_probe: Callable[[], CheckResult],
    analytics_probe: Optional[Callable[[], CheckResult]] = None,
) -> List[CheckResult]:
    results: List[CheckResult] = []

    results.append(org_probe())

    if analytics_probe:
        results.append(analytics_probe())
